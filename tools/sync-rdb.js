...
import EntityDal from '../src/server/lib/repositories/EntityRepository';
import rethinkDbFireStarter from './lib/rethinkDbFireStarter';
import colors from 'colors';

let entityDal = new EntityDal(rc.DB_DEFAULT);

rh.connect((error, connection: Connection) => {
    if (error) {
        throw error;
    }
    console.log(colors.green('RethinkDB connected.'));
    console.log(colors.green('Setting up the database...'));
    async.series([
        ...
        if(error) {
            ...
            console.log(colors.green('Everything is in sync'));
        }
        connection.close();
    });
});